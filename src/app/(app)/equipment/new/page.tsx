...
interface Type { id: number; name: string; category: string; }
...
export default function NewEquipmentPage() {
  ...
  const initialSubId = searchParams.get('substation_id') ?? '';
  ...
  useEffect(() => {
    fetch('/api/equipment-types').then((r) => r.json()).then((d) => setTypes((d.rows ?? []).filter((t: Type & { active: number }) => t.active !== 0)));
  ...
  useEffect(() => {
    ...
    if (!t) { setRatings([]); return; }
    const fields = getSpecFields(t.name);
    setRatings(fields.map((f) => ({ spec_key: f.key, spec_label: f.label, spec_value: '', unit: f.unit })));
  }, [form.equipment_type_id, types]);
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.equipment_tag || !form.equipment_name || !form.equipment_type_id || !form.substation_id) {
      toast.error('Tag, name, type and substation are required');
      return;
    }
    setBusy(true);
    const r = await fetch('/api/equipment', {
      ...
      body: JSON.stringify({
        ...
        ratings: ratings.filter((r) => r.spec_value !== '' || r.spec_key),
      }),
    });
    setBusy(false);
    if (!r.ok) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 p-4 md:p-6">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Cpu className="h-4 w-4 text-primary" />1 · Basic information</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <Field label="Equipment tag" required full>
              <Input value={form.equipment_tag} onChange={(e) => setField('equipment_tag', e.target.value.toUpperCase())} placeholder="e.g. WT-RAI-001" required />
            </Field>
            ...
            <Field label="Equipment type" required>
              <Select value={form.equipment_type_id || 'none'} onValueChange={(v) => setField('equipment_type_id', v === 'none' ? '' : v)}>
            ...
            <Field label="Substation" required>
              <Select value={form.substation_id || 'none'} onValueChange={(v) => setField('substation_id', v === 'none' ? '' : v)}>
                <SelectTrigger><SelectValue placeholder="Select substation" /></SelectTrigger>
                <SelectContent>
            ...
            <Field label="Voltage level"><Input value={form.voltage_level} onChange={(e) => setField('voltage_level', e.target.value)} placeholder="e.g. 132 kV" /></Field>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle className="text-base">2 · Manufacturer information</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <Field label="Manufacturer"><Input value={form.manufacturer} onChange={(e) => setField('manufacturer', e.target.value)} /></Field>
            <Field label="Model number"><Input value={form.model_number} onChange={(e) => setField('model_number', e.target.value)} /></Field>
            <Field label="Serial number"><Input value={form.serial_number} onChange={(e) => setField('serial_number', e.target.value)} /></Field>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle className="text-base">3 · Installation</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">4 · Technical specifications</CardTitle>
          </CardHeader>
          <CardContent>
            {ratings.length === 0 ? (
              <div className="rounded border border-dashed py-6 text-center text-xs text-muted-foreground">
                Select an equipment type to load default specification fields. You can also add custom fields below.
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                {ratings.map((r, idx) => (
                  <Field key={r.spec_key} label={`${r.spec_label}${r.unit ? ` (${r.unit})` : ''}`}>
                    <div className="flex gap-1">
                      <Input value={r.spec_value} onChange={(e) => updateRating(idx, e.target.value)} placeholder="Not specified" />
                      <Button type="button" size="icon" variant="ghost" onClick={() => removeRating(idx)} className="h-9 w-9">
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">5 · Condition & Criticality</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">6 · Maintenance</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <Field label="Maintenance frequency (days)"><Input type="number" min={1} max={3650} value={form.maintenance_frequency_days} onChange={(e) => setField('maintenance_frequency_days', Number(e.target.value))} /></Field>
            ...
            <Field label="Responsible person" full><Input value={form.responsible_person} onChange={(e) => setField('responsible_person', e.target.value)} /></Field>
          </CardContent>
        </Card>

        <div className="flex justify-end gap-2">
          <Button asChild variant="outline" type="button"><Link href="/equipment">Cancel</Link></Button>
          <Button type="submit" disabled={busy}>{busy ? 'Saving…' : 'Create equipment'}</Button>
        </div>
      </form>
    </div>
  );
